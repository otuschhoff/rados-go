...
ceph_cli auth get-or-create mgr.p12 mon 'allow profile mgr' osd 'allow *' mds 'allow *' -o /cluster/mgr-p12.keyring
mkdir -p "$temporary/mgr-p12"
cp "$temporary/mgr-p12.keyring" "$temporary/mgr-p12/keyring"
chmod 600 "$temporary/mgr-p12/keyring"
docker run -d --name "p12-mgr-$$" --platform "$platform" --network "$network" --ip 172.30.112.11 -v "$temporary:/cluster" "$image" \
	ceph-mgr -f -i p12 --conf /cluster/ceph.conf --mgr-data /cluster/mgr-p12 --keyring /cluster/mgr-p12/keyring --setuser ceph --setgroup ceph --log-to-stderr true --err-to-stderr true --log-file '' >/dev/null
for attempt in $(seq 1 60); do
	if ceph_cli mgr dump --format json 2>/dev/null | jq -e '.active_name == "p12"' >/dev/null; then break; fi
	test "$attempt" -lt 60 || { docker logs "p12-mgr-$$" >&2; exit 1; }
	sleep 1
done
