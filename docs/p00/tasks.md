...
passed on Linux arm64 against Ceph 20.2.4, produced a schema-valid report, and
cleaned the cluster and loop devices. Oracle builds are verified for amd64 and
arm64. **Handoff:** retain the sanitized report and rerun when any pinned input
...
complete, both oracle architectures build, and the live smoke report passes.
Final independent review is pending. Review ownership is recorded by role in
