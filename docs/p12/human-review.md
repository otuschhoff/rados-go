...
The reviewer trust policy is anchored outside the candidate repository. Before
verifying signatures, obtain the approved lowercase SHA-256 of
`reviewer-trust.json` through the release organization's protected channel and
set `P12_REVIEWER_TRUST_SHA256` to that value. `make verify-p12-review` and final
candidate verification reject a missing, malformed, or mismatched external
digest. Do not derive the trusted value from the candidate checkout being
verified.
