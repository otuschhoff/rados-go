...
func TestReviewerTrustRequiresExternalDigest(t *testing.T) {
	path := t.TempDir() + "/reviewer-trust.json"
	data := []byte("trusted policy\n")
	if err := os.WriteFile(path, data, 0o644); err != nil {
		t.Fatal(err)
	}
	digest := sha256.Sum256(data)
	expected := hex.EncodeToString(digest[:])
	if err := validateTrustPolicyDigest(path, expected); err != nil {
		t.Fatalf("matching external digest rejected: %v", err)
	}
	for _, invalid := range []string{"", strings.ToUpper(expected), strings.Repeat("0", 64)} {
		if err := validateTrustPolicyDigest(path, invalid); err == nil {
			t.Fatalf("invalid external digest accepted: %q", invalid)
		}
	}
}
...
func TestApprovalReferenceRequiresHTTP(t *testing.T) {
	for _, value := range []string{"https://approvals.example.test/p12/1", "http://approvals.example.test/p12/1"} {
		if !validApprovalReference(value) {
			t.Fatalf("valid approval reference rejected: %q", value)
		}
	}
	for _, value := range []string{"ftp://approvals.example.test/p12/1", "urn:approval:p12:1", "https://"} {
		if validApprovalReference(value) {
			t.Fatalf("invalid approval reference accepted: %q", value)
		}
	}
}
