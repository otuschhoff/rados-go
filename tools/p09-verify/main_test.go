...
func TestReadReportRejectsUnknownFieldsWithoutPanic(t *testing.T) {
	if os.Getenv("P09_VERIFY_MALFORMED_HELPER") == "1" {
		readReport(os.Getenv("P09_VERIFY_REPORT"))
		return
	}
	...
	command.Env = append(os.Environ(), "P09_VERIFY_MALFORMED_HELPER=1", "P09_VERIFY_REPORT="+path)
	output, err := command.CombinedOutput()
	if err == nil {
		t.Fatal("malformed report accepted")
	}
	text := string(output)
	if !strings.Contains(text, "decode P09 report") || strings.Contains(text, "panic:") {
